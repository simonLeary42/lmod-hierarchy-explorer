...
for path in ["/home/simonleary_umass_edu/ondemand/dev/modules/make-json/x86_64"]:
    if not os.path.isfile(path):
        raise RuntimeError(f"\"{path}\" is not a file!")
    with open(path, 'r') as json_file:
        json_data = json.load(json_file)
    for module_name, module_info in json_data.items():
        for modulefile, modulefile_info in module_info.items():
            ...
            if modulefile_info["hidden"]:
                nested_dict_append(hidden_modules, parent_dir, name, version)
            else:
                nested_dict_append(non_hidden_modules, parent_dir, name, version)

# hide the hidden directories
for dir in HIDDEN_PARENT_DIRS:
    hidden_modules[dir] = non_hidden_modules[dir]
    non_hidden_modules.pop(dir)

# remove any version that is just "latest"
for _dict in [hidden_modules, non_hidden_modules]:
    for parent_dir, name2versions in _dict.items():
        for name, versions in name2versions.items():
            if "latest" in versions:
                _dict[parent_dir][name] = [x for x in versions if x != "latest"]
...
for _dict in [hidden_modules, non_hidden_modules]:
    for parent_dir, name2versions in _dict.items():
        for name, versions in name2versions.items():
            if len(versions) > 1:
                _dict[parent_dir][name] = [*set(versions)]
...
for _dict in [hidden_modules, non_hidden_modules]:
    for parent_dir, name2versions in _dict.items():
        for name, versions in name2versions.items():
            if len(versions) == 1:
                _dict[parent_dir][name] = versions[0]

# put parent directories in order of how many modules they provide
for _dict in [hidden_modules, non_hidden_modules]:
    _dict = dict(sorted(_dict.items(), key=lambda item: len(item[1])))

# put modules in alphabetical order
for _dict in [hidden_modules, non_hidden_modules]:
    for parent_dir, name2versions in _dict.items():
        #_dict[parent_dir] = dict(sorted(name2versions.items(), key=lambda item: item[0]))
        _dict[parent_dir] = dict(sorted(name2versions.items()))

with open("hierarchy.json", 'w') as json_out_file:
    json.dump(non_hidden_modules, json_out_file)
